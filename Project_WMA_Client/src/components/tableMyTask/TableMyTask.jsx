...
import './tableMyTask.css'; // Import CSS file for styling

const MyTask = () => {
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);
  const [tasks, setTasks] = useState([
    { key: 1, dueDate: null, priority: ['High'], taskProcess: ['In Progress'], description: 'Description 1', profile: 'Profile 1' },
    { key: 2, dueDate: null, priority: ['Medium'], taskProcess: ['Not Started'], description: 'Description 2', profile: 'Profile 2' },
    { key: 3, dueDate: null, priority: ['Low'], taskProcess: ['Done'], description: 'Description 3', profile: 'Profile 3' },
  ...
  const handleProfileChange = (e, record) => {
    console.log('Profile changed: ', e.target.value, ' for record: ', record);
    const updatedTasks = tasks.map(task => {
      if (task.key === record.key) {
        return { ...task, profile: e.target.value };
      }
      return task;
    });
    setTasks(updatedTasks);
  ...
  const handleAddTask = () => {
    ...
    const newTask = { key: newKey, dueDate: null, priority: ['Medium'], taskProcess: ['Not Started'], description: '', profile: '' };
        ...
        <Table.Column
          ...
          render={(text, record) => (
            <Input
              ...
              onChange={(e) => handleProfileChange(e, record)}
            />
          )}
        />
      </Table>
